...
impl ToTokens for FlatExpansion<'_> {
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let constrained_generics = self
            .self_ty
            .1
            .type_params()
            .map(|param| &param.ident)
            .map(|param| quote! { #param: derive_more::core::str::FromStr });
        ...
        let (error_type, fallback_constructor) = self
            ...
            .map(|m| {
                (quote! { derive_more::core::convert::Infallible }, {
        ...
        quote! {
            ...
            impl #impl_generics derive_more::core::str::FromStr for #ty #ty_generics #where_clause {
                ...
                fn from_str(
                    s: &str,
                ) -> derive_more::core::result::Result<Self, #error_type> {
                    #(#forwarded)*
